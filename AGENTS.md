...
- **Marketing consent + suppression** (migration `0011`): LFPDPPP-grade channel-scoped consent (`consent_records` + `consent_audit`, statuses granted/revoked/pending_double_opt_in, state machine in `packages/services/src/consent/consent-state-machine.ts`) and cross-product suppression list (`suppression_entries`; **suppression always wins over consent**). Double opt-in: SHA-256-hashed token, 7-day TTL, confirm at `GET /api/consent/confirm?token=`. External capture API for dhanam/karafiel/tezca under `/api/v1/consent/{capture,check,suppression/{add,check,list}}` (HMAC `PHYND_CONSENT_EVENTS_SECRET`) — contract in `docs/CONSENT_API.md`. Send gate precedence: suppression → channel consent record → legacy `contacts.marketingConsent` fallback; Tulana GA gates unchanged
- **Email open/click tracking** (migration `0011`): Resend event webhook `POST /api/webhooks/resend` (Svix signature, `RESEND_WEBHOOK_SECRET`) → `campaign_email_events` (dedup by svix message id) via `CampaignEmailEventService`; opens/clicks on SKU campaigns become buyer signals (Tulana export); bounces/complaints auto-add suppression entries. Drip sends tag emails (`lead_id`/`contact_id`/`drip_step`) and persist `sent` rows
- **Structured campaign handoff + preheaders** (migration `0012`): `POST /api/v1/campaigns/import` additionally accepts `draft_variants: (string | {variant_id, language, subject, preheader?, body, cta?, claim_keys_used[]})[]` matching Selva's generate-copy output; legacy `list[str]` and the existing `drafts` field keep working. Variants persist to `campaign_draft_variants` (claims audit trail survives draft→approved) via `CampaignDraftVariantService`; surfaced in the Tulana review dialog through `campaigns.listDraftVariants`. Email layer supports preview text: `SendEmailParams.preheader` → hidden-preheader div injected after `<body>` (`email/preheader.ts`); drip templates return `preheader`; `campaignVariantEmail` renders a structured variant
