...
test.describe('Bulk Operations', () => {
  test.describe('contacts table', () => {
    test('unauthenticated users are redirected from contacts to login', async ({ page }) => {
      test.skip(process.env.AUTH_BYPASS === 'true', 'AUTH_BYPASS skips redirect')
      await page.goto('/contacts')
      await expect(page).toHaveURL(/\/login/)
    })

    test('contacts table renders with selectable checkboxes', async ({ page }) => {
    ...
    test('individual row checkboxes are visible in contacts table', async ({ page }) => {
      // Requires: authenticated session + seeded contacts
      await page.goto('/contacts')
    ...
    test('selecting a contact row shows bulk actions toolbar', async ({ page }) => {
      // Requires: authenticated session + seeded contacts
      await page.goto('/contacts')
      ...
      const firstCheckbox = page.locator('[aria-label^="Select row"]').first()
      await firstCheckbox.click()
      // Toolbar should appear with "1 selected" text
      await expect(page.getByText('1 selected')).toBeVisible()
    })

    test('select all checkbox selects all visible contact rows', async ({ page }) => {
      // Requires: authenticated session + seeded contacts
      await page.goto('/contacts')
    ...
    test('contacts export CSV button is visible when rows are selected', async ({ page }) => {
      // Requires: authenticated session + seeded contacts
      await page.goto('/contacts')
  ...
  test.describe('leads table', () => {
    ...
    test('leads table renders with selectable checkboxes', async ({ page }) => {
    ...
    test('selecting lead rows shows bulk actions toolbar with status change option', async ({
      page,
    }) => {
      // Requires: authenticated session + seeded leads
      await page.goto('/leads')
    ...
    test('selecting lead rows shows status selector dropdown', async ({ page }) => {
      // Requires: authenticated session + seeded leads
      await page.goto('/leads')
      ...
      await expect(page.getByText('Select status...')).toBeVisible()
    })

    test('bulk status change updates selected leads', async ({ page }) => {
      ...
      await expect(page.getByText('Leads updated')).toBeVisible()
    })
  })

  test.describe('opportunities table', () => {
    test('unauthenticated users are redirected from opportunities to login', async ({ page }) => {
      test.skip(process.env.AUTH_BYPASS === 'true', 'AUTH_BYPASS skips redirect')
      await page.goto('/opportunities')
      await expect(page).toHaveURL(/\/login/)
    })

    test('opportunities table renders with selectable checkboxes', async ({ page }) => {
    ...
    test('selecting opportunity rows shows bulk actions toolbar', async ({ page }) => {
      // Requires: authenticated session + seeded opportunities
      await page.goto('/opportunities')
      const firstCheckbox = page.locator('[aria-label^="Select row"]').first()
      await firstCheckbox.click()
    ...
    test('deselecting all rows hides bulk actions toolbar', async ({ page }) => {
      // Requires: authenticated session + seeded opportunities
      await page.goto('/opportunities')
      const firstCheckbox = page.locator('[aria-label^="Select row"]').first()
      // Select
  ...
  test.describe('data table interaction patterns', () => {
    test('select-all with search filter only selects visible rows', async ({ page }) => {
    ...
    test('selected rows have data-state="selected" attribute', async ({ page }) => {
      // Requires: authenticated session + seeded contacts
      await page.goto('/contacts')
      const firstCheckbox = page.locator('[aria-label^="Select row"]').first()
      await firstCheckbox.click()
