...
- `crm.phyne.app/` follows the same authenticated app-host behavior for non-MADFAM tenants.
...
- `status.madfam.io` should track `crm.phyne.app` as the generic app host. Until DNS and Enclii routing exist, that entry should truthfully show outage rather than being hidden or represented by `app.phynd.app`.
...
- `https://crm.phyne.app/` does not resolve publicly yet, so it is not production-functional.
