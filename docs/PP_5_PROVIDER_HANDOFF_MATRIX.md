...
| `JANUA_TELEMETRY_WEBHOOK_SECRET` currently falls back to `JANUA_WEBHOOK_SECRET`. | Janua telemetry + PhyndCRM | Keep distinct staging secret installed; remove reliance on fallback after producers are split. |
