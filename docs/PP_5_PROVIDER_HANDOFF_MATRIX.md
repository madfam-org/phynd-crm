...
| Dhanam billing | Dhanam | `POST /api/webhooks/dhanam` | `DHANAM_WEBHOOK_SECRET`, `DHANAM_API_URL` | Add staging billing relay destination with `X-Dhanam-Signature`; use sandbox billing data only. | Send paid, failed, refunded, disputed, and cancelled events; PhyneCRM writes `webhook_events`, conversions, optional referral/lead updates for paid events, order payment reconciliation/lifecycle references, and engagement timeline events in staging only. | External handoff |
...
| Engagement events | Cotiza / Pravara / Selva / Karafiel / Dhanam | `POST /api/v1/engagements/events` | `PHYNE_ENGAGEMENT_EVENTS_SECRET` | Add staging event destination for project milestone/status writes. | Event with `engagement_id`, `source`, `event_type` records timeline entry; duplicate `dedup_key` dedupes. Paid order dispatch intent creates `system:production_dispatch_requested` before live provider calls. | External handoff |
