...
End-to-end staff workflow for onboarding an external client (tablaco being the first) through MADFAM's cross-platform engagement flow: Cotiza (quoting) → PhyndCRM (engagement + portal) → Janua (magic link) → Pravara (fab dispatch, optional) → Dhanam (billing).
...
2. **PhyndCRM env vars** (prod + staging):
...
### Step 1 — Create the engagement (PhyndCRM)
...
POST /api/v1/quotes
{
  ...
  "quoteType": "services",
  "metadata": {
    "phyndcrmEngagementId": "<engagement_id from step 1>"
  }
}
```
...
This fires two fire-and-forget side effects to PhyndCRM:
...
If either side-effect fails (PhyndCRM offline, S3 hiccup), the approve flow still succeeds. Staff can manually add the artifact via `engagements.addArtifact` if needed:
...
| Symptom | Check | Fix |
|---|---|---|
| Cotiza `approve()` doesn't create PhyndCRM event | `PHYNDCRM_API_URL` / `PHYNDCRM_ENGAGEMENT_SECRET` set? Engagement ID in quote metadata? | Set env vars + re-approve (engagement event dedupes; harmless) |
...
| `AUTHZ_MISMATCH` when client clicks | Contact email in PhyndCRM differs from what Janua verified | Update the contact email OR send a fresh link to the correct address |
...
- ~~Staff UI for all of the above (replace tRPC CLI calls)~~ — ✅ shipped (phynd-crm#12): `/engagements` list + detail pages, "Send portal link" button, "Add artifact" dialog, engagements section on `/clients/[id]`.
