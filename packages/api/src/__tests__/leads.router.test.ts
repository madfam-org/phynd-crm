...
describe('leads router', () => {
  ...
  it('list accepts cursor and limit', async () => {
    const ctx = createMockCtx()
    const caller = createCaller(ctx)
    const result = await caller.leads.list({ cursor: 'abc', limit: 5 })
  ...
  it('listMine returns leads owned by current user', async () => {
    const ctx = createMockCtx()
    const caller = createCaller(ctx)
    const result = await caller.leads.listMine()
    expect(result).toEqual({ hasMore: false, items: [], nextCursor: null })
  })

  it('listByContactId returns leads for a contact', async () => {
    const ctx = createMockCtx()
    const caller = createCaller(ctx)
    const result = await caller.leads.listByContactId({
  ...
  it('create accepts valid input', async () => {
    const ctx = createMockCtx()
    // create() uses tx.insert().values().returning() and accesses lead.id
    const qb = (ctx.db as unknown as { insert: ReturnType<typeof vi.fn> }).insert()
    qb._result = [{ id: 'lead-001', pipelineId: 'p1', stageId: 's1' }]
    const caller = createCaller(ctx)
    await expect(
      caller.leads.create({
  ...
  it('bulkUpdateStatus accepts up to 100 ids', async () => {
    ...
    await expect(caller.leads.bulkUpdateStatus({ ids, status: 'qualified' })).resolves.not.toThrow()
  })
  ...
  it('bulkUpdateStatus rejects more than 100 ids', async () => {
    ...
    await expect(caller.leads.bulkUpdateStatus({ ids, status: 'qualified' })).rejects.toThrow()
  })
})
