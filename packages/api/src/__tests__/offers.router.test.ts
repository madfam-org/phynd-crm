...
describe('offers router', () => {
  ...
  it('list returns paginated offers', async () => {
  ...
  it('create accepts name and type', async () => {
    const ctx = createMockCtx()
    const caller = createCaller(ctx)
    await expect(
  ...
  it('update accepts status change', async () => {
    const ctx = createMockCtx()
    const caller = createCaller(ctx)
    await expect(
  ...
  it('redeem accepts id', async () => {
    const ctx = createMockCtx()
    const caller = createCaller(ctx)
    await expect(
      caller.offers.redeem({ id: '00000000-0000-0000-0000-000000000001' }),
    ).resolves.not.toThrow()
  })
  ...
  it('delete accepts id', async () => {
    ...
    const caller = createCaller(ctx)
    await expect(
      caller.offers.delete({ id: '00000000-0000-0000-0000-000000000001' }),
    ).resolves.not.toThrow()
  })
})
