...
function resolveEvidenceUrl(campaignId: string): string {
  const baseUrl =
    process.env.PHYND_CRM_PUBLIC_URL ||
    process.env.NEXT_PUBLIC_APP_URL ||
    process.env.APP_URL ||
    'https://phynecrm.madfam.io'
...
export async function recordTulanaCommercialGaG4Evidence(
  ...
  const period =
    metadataString(input.tulanaMetadata, 'commercial_ga_period') ||
    process.env.TULANA_COMMERCIAL_GA_PERIOD
  if (!period) {
    return { attempted: false, status: 'skipped', reason: 'missing_commercial_ga_period' }
  }

  const environment =
    metadataString(input.tulanaMetadata, 'commercial_ga_environment') ||
  ...
  const humanApproverEmail = metadataString(input.tulanaMetadata, 'human_approver_email') || ''
  ...
  const body = {
    ...
    approved_by_email: humanApproverEmail,
  ...
  try {
    const response = await fetch(
      `${apiBaseUrl}/madfam-skus/${encodeURIComponent(sku.productSlug)}/${encodeURIComponent(
        sku.tierSlug,
